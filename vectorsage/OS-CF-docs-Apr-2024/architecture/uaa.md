...
when they act on behalf of Cloud Foundry users.
In collaboration with the login server, UAA can authenticate users with their Cloud Foundry credentials, and can act as an SSO service using those, or other, credentials.
...
You can deploy UAA locally or to Cloud Foundry.
...
4. If successful, the three apps run together on a single instance of Tomcat listening on port 8080, with endpoints `/uaa`, `/app`, and `/api`. Tomcat is a library of Java code used by UAA to create its features. For more information, see the [Apache Tomcat](http://tomcat.apache.org/) website.

### Using local UAA
To access and use a locally deployed UAA server:

1. Run the UAA server as described in [Deploy Locally](https://docs.cloudfoundry.org/concepts/architecture/uaa.html#local).
...
3. To install the Cloud Foundry UAA Command Line Client (UAAC) Ruby gem, run:
```
gem install cf-uaac
```
...
To build the UAA as an app and push it to Cloud Foundry using the Cloud Foundry Command Line Interface (cf CLI):
...
4. Run the cf CLI `cf push APP-NAME -m 512M -p PATH-TO-WAR-FILE --no-start` command to push the app to Cloud Foundry. Replace `APP-NAME` with a name for your UAA app, and `PATH-TO-WAR-FILE` with the path to the WAR file you created in the previous step. For example:
```
...
To access and use a UAA server that you pushed as an app to Cloud Foundry:

1. Follow the procedure in [Deploy UAA to Cloud Foundry](https://docs.cloudfoundry.org/concepts/architecture/uaa.html#cf).
...
* `USERNAME` is a valid username. For example, `[dev@example.org](mailto:dev@example.org)`.

* `PASSWORD` is the password for the previous username.

* `EMAIL-ADDRESS` is the email address for the previous user. Example: `[dev@example.org](mailto:dev@example.org)`.
...
### Scopes of privilege
The following table describes the scopes of privilege in other Cloud Foundry services.
| Scope | Description |
| --- | --- |
| `doppler.firehose` | This scope gives the ability to read logs from the [Loggregator Architecture](https://docs.cloudfoundry.org/loggregator/architecture.html) endpoint. |
| `notifications.write` | This scope gives the ability to send notifications through the [Notification Service](https://docs.cloudfoundry.org/adminguide/notifications.html). |